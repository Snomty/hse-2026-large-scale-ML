...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--grad-accum-steps', default=1, type=int, help='Gradient accumulation steps')
    return parser
...
def main(args: argparse.Namespace) -> None:  # noqa: C901, PLR0915, PLR0912
    ...
    for state['epoch'] in range(state['epoch'], args.num_epochs):  # noqa: B020
        ...
        for i_step in range(len(dataloader)):
            ...
            if (state['epoch_step'] + 1) % args.grad_accum_steps == 0:
                with timers['update']:
                    optimizer.step()
                    lr_scheduler.step()
            ...
            if state['epoch_step'] % args.log_freq == 0 and state['epoch_step'] > 0:
